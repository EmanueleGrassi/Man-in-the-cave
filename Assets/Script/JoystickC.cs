...
public class JoystickC : MonoBehaviour
{
	...
	public void ResetJoystick ()
	{
		...
		if (touchPad)			
			gui.color = new Color (gui.color.r, gui.color.g, gui.color.b, 0.275f);
	}
	...
	{	
			...
			for (int i = 0; i < count; i++) {
				...
				if (lastFingerId == touch.fingerId) {	
					...
					if (touchPad) {	
						...
						position = new Vector2 (Mathf.Clamp ((touch.position.x - fingerDownPos.x) / (touchZone.width / 2), -1, 1),
		...
		if (!touchPad) {
			...
			position = new Vector2 ((gui.pixelInset.x + guiTouchOffset.x - guiCenter.x) / guiTouchOffset.x,
